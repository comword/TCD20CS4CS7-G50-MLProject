...
function createsqlfor(CountryCode,IndCode,Year,Data){
  #print CountryCode,IndCode,Year,Data
  print "INSERT INTO \"data\" (\"CountryCode\",\"Year\") SELECT \"" CountryCode "\"," Year " WHERE NOT EXISTS (SELECT 1 FROM \"data\" WHERE \"CountryCode\" == \"" CountryCode "\" AND \"Year\" == " Year ");"
  print "UPDATE \"data\" SET \"" IndCode "\" = " Data " WHERE \"CountryCode\" == \"" CountryCode "\" AND \"Year\" == " Year ";"
}

{
  ...
  if(FNR==1){
    ...
    for (keyw = 0;keyw < num_fields-1;keyw++) {
      if(keyw>=4){
        Year=csv[keyw]
        Data=csvw[keyw]
        if (Data != ""){
          createsqlfor(CountryCode,IndCode,Year,Data)
        }
